...
namespace WorkerTest
{
    public partial class WorkerTest : Form
    {
        ...
        {
            if (MessageBox.Show("Confirm to import", "Confirm import data?", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No) return;
        ...
        private void DumpToElastic(string tType, string tXml)
        {
            string url = txtUrl.Text;
            string urlType = url + tType + "/";

            var httpWebRequest = (HttpWebRequest)WebRequest.Create(urlType);
                    ...
                    while ((line = sr.ReadLine()) != null)
                    {
                        ...
                        else
                        {
                            if (line.Length > 1)
                            {
                                ...
                                //txtType.Text = arLine[0];

                                //int percentage = (int)((counter / (double)line.Length) * 100.0);
                                //this.progressBar1.Value = percentage;
                                DumpToElastic(arLine[0], arLine[1]);
                                bytesProcessed += 1;
        ...
        private void btnMgrDb_Click(object sender, EventArgs e)
        {
